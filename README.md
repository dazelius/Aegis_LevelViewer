...
| `AEGISGRAM_GIT_URL_REWRITES`    | runtime     | Whitespace-separated `<from> <to>` pairs injected into every git/LFS subprocess as `url.<to>.insteadOf=<from>`. Use when the GitLab LFS batch API returns download URLs on a host the deploy server cannot reach. Example: `http://13.209.114.157/ http://172.31.2.91/`. If your deploy platform rewrites env values containing public IPs (UAAutoTool does this), prefix the value with `base64:` and paste the base64-encoded pair list instead, e.g. `base64:aHR0cDovLzEzLjIwOS4xMTQuMTU3LyBodHRwOi8vMTcyLjMxLjIuOTEv` for the example above. Multiple rewrites: keep listing pairs. Legacy `from=to,from=to` shape still accepted. |
